...
/**
 * Creates the `tool.execute.after` hook that:
 * 1. Transforms Read tool output to hashline LINE#ID format
 * 2. Replaces Write tool output with a line-count summary
 *
 * Both behaviors are gated by `pluginConfig.hashline_edit !== false`.
 */
export function handleToolExecuteAfter(pluginConfig: OcBlackbytesConfig) {
  const enabled = pluginConfig.hashline_edit !== false
